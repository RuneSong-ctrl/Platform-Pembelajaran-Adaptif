...
export default function UnitVisual({ visual, sources, onChange, disabled }: Props) {
  ...
  const field = (label: string, value: string, maxLength: number, update: (value: string) => void) =>
    <label className="block text-sm">{label}<textarea disabled={disabled} maxLength={maxLength} className="block border rounded-lg w-full p-2 text-slate-900 bg-white" value={value} onChange={event => update(event.target.value)} /></label>;
  ...
  return <section className="min-w-0 rounded-xl border border-slate-200 bg-slate-50 p-4 space-y-4 text-slate-900" aria-label={names[visual.kind]}>
    <div><p className="text-xs font-semibold uppercase tracking-wide text-slate-600">{names[visual.kind]}</p><h4 className="font-bold text-lg break-words">{visual.title}</h4></div>
    {visual.kind === "concept_map" && <>
      <p className="text-sm text-slate-600">Panah menunjukkan arah hubungan. Baca makna setiap hubungan pada daftar di bawah peta.</p>
      <div className="hidden sm:block rounded-lg bg-white border border-slate-200">
        <svg viewBox="0 0 1000 800" className="w-full max-w-[700px] mx-auto" aria-hidden="true">
          ...
          {nodes.map((node, i) => <foreignObject key={node.id} x={positions[i].x - 95} y={positions[i].y - 75} width="190" height="150">
            <div className="h-full flex items-center justify-center rounded-xl border border-slate-400 bg-white p-3 text-center text-base leading-5 font-semibold break-words">{node.label}</div>
          </foreignObject>)}
        </svg>
      </div>
      <ul className="space-y-3" aria-label="Hubungan antarkonsep">{links.map((link, i) => <li key={i} className="rounded-lg border border-slate-200 bg-white p-3 break-words">
    ...
    {visual.kind !== "comparison" && <ol className="space-y-3" aria-label={visual.kind === "sequence" ? "Langkah berurutan" : "Penjelasan konsep"}>
      {nodes.map((node, i) => <li key={node.id} className="flex gap-3 min-w-0">
        {visual.kind === "sequence" && <div className="flex flex-col items-center" aria-hidden="true"><span className="flex h-9 w-9 items-center justify-center rounded-full border border-slate-400 bg-white font-bold">{i + 1}</span>{i < nodes.length - 1 && <span className="mt-2 flex-1 border-l border-slate-400" />}</div>}
        <details className="flex-1 min-w-0 rounded-lg border border-slate-200 bg-white p-3">
    ...
    {visual.kind === "comparison" && <>
      <div className="sm:hidden space-y-3">{rows.map((row, i) => <section key={i} className="rounded-lg border border-slate-200 bg-white p-3">
      ...
      <table className="w-full min-w-[560px] table-fixed border-collapse text-sm bg-white">
        <caption className="text-left pb-2 text-slate-600">Bandingkan objek pada aspek yang sama; kutipan tersedia pada setiap baris.</caption>
    ...
    {onChange && <details className="border-t border-slate-300 pt-3">
      <summary className="cursor-pointer font-semibold py-2">Edit visual</summary>
      <div className="space-y-4 mt-3">
        <p className="text-sm text-slate-600">Perubahan harus tetap didukung kutipan yang tercantum. Simpan sebelum menyetujui. Buat ulang draf jika struktur atau sumbernya tidak sesuai.</p>
