...
const labels: Record<string, string> = {
  NOT_GENERATED: "Isi belajar belum dibuat", PROCESSING: "Sedang disusun, tunggu sebentar", DRAFT: "Siap Anda periksa", ERROR: "Gagal disusun, coba lagi",
};
...
export default function LearningUnits({ documentId, teacher = false, showVisual = false }: { documentId: string; teacher?: boolean; showVisual?: boolean }) {
  ...
  return <section className="clay-card p-5 sm:p-6 space-y-4" aria-label="Unit belajar bersumber">
    <h2 className="text-[18px] leading-[26px] font-bold text-[#1C1E26]">{teacher ? "Isi belajar dari materi ini" : "Konsep dari materi guru"}</h2>
    ...
    {data && <>
      <p role="status" className="text-sm text-[#475569]">{teacher ? labels[data.state] || data.state : data.units.length ? "Materi telah disetujui guru." : "Unit belajar belum dipublikasikan oleh guru."}
        {teacher && data.published_revision != null && ` · Sudah terbit (versi ${data.published_revision})`}
      </p>
      {teacher && data.error && <p className="text-[#852C28]">{data.error}</p>}
      {teacher && <div className="flex flex-wrap gap-2">
        <button className="clay-btn clay-btn-white px-3.5 py-2 text-xs font-black cursor-pointer disabled:opacity-50" disabled={busy || dirty} onClick={() => void action("generate")}>{data.state === "PROCESSING" ? "Ulangi jika macet lebih dari 30 menit" : data.units.length ? "Susun ulang" : "Susun isi belajar"}</button>
        <button className="clay-btn clay-btn-white px-3.5 py-2 text-xs font-black cursor-pointer disabled:opacity-50" disabled={disabled || !dirty} onClick={() => void action("save")}>Simpan perubahan</button>
        <button className="clay-btn clay-btn-dark px-3.5 py-2 text-xs font-black cursor-pointer disabled:opacity-50" disabled={disabled || dirty || data.state !== "DRAFT" || !data.units.length} onClick={() => void action("approve")}>Setujui &amp; tampilkan ke siswa</button>
      </div>}
      {teacher && <p className="text-xs text-[#475569]">Isi di bawah disusun otomatis dari materi Anda. Cocokkan penjelasan dan kunci jawaban dengan kutipan aslinya sebelum menyetujui, karena susunan otomatis bisa keliru.</p>}
      {teacher && data.infographic && (
        <div className="rounded-2xl border border-[#E6E4EE] bg-[#F7F6FA] p-4 space-y-2" aria-live="polite">
          <p className="text-sm font-semibold text-[#1C1E26]">Gambar pendukung infografis</p>
          {!data.image && <p className="text-sm text-[#475569]">Belum ada gambar untuk draf ini.</p>}
          {data.image?.state === "PROCESSING" && <p role="status" className="text-sm text-[#475569]">Gambar sedang dibuat (sekitar 15–40 detik)…</p>}
          ...
          {data.image?.state === "READY" && (data.image.lettered?.length
            ? <p className="text-sm text-[#785308]">Gambar ini mungkin masih ada tulisan yang tidak perlu: {data.image.lettered.join(", ")}. Buat ulang atau hapus sebelum menyetujui.</p>
            : <p className="text-sm text-[#1D5E4D]">Gambar sudah dicek: tidak ada tulisan tambahan. Semua teks di infografis diambil dari materi Anda.</p>)}
      ...
      {teacher && data.units.length > 0 && !data.infographic && (
        <p className="text-sm text-[#475569]">Infografis belum ada. Klik "Susun ulang" untuk membuatnya.</p>
      )}
      {teacher && data.units.length > 0 && (
        <details className="rounded-2xl border border-[#785308]/25 bg-[#FFF6DF] p-3">
          <summary className="cursor-pointer font-semibold py-1">Lihat permainan untuk siswa</summary>
          <p className="text-xs text-[#475569] my-2">Permainan ini dibuat dari isi di bawah. Siswa mengerjakannya bertahap; tahap berikutnya terbuka setelah tahap sebelumnya selesai.</p>
      ...
      {data.units.map((unit, index) => <article key={unit.id} className="border-t border-[#E6E4EE] pt-4 space-y-3">
        {teacher ? <>
          ...
          <label className="block text-sm">Tujuan belajar<textarea disabled={disabled} className="block border rounded-lg w-full p-2" value={unit.learning_objective} onChange={event => change(index, { learning_objective: event.target.value })} /></label>
        ...
        {(teacher || showVisual) && (unit.visual
          ...
          : <p className="text-sm text-[#475569]">Visual belum tersedia untuk unit ini. Konsep dan sumber tetap dapat dibaca.{teacher && " Klik \"Susun ulang\" untuk mencoba membuat gambar lagi."}</p>)}
        ...
        {teacher && <details><summary className="cursor-pointer">Periksa soal latihan dan kunci jawaban</summary>
